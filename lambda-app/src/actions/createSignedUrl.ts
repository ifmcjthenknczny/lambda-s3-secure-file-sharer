...
import { findSecretCode, SecretCode, useSecretCode } from '../client/secretCodes'
...
type CodeValidationResult =
    ...
    | { isValid: false; errorResponse: LambdaResponse };
...
async function validateSecretCode(
    ...
    if (dayjs(now).isAfter(dayjs(dbCode.expiresAt))) {
        return {
            ...
            errorResponse: {
                statusCode: 403,
                body: JSON.stringify({ message: 'Access expired (code has passed its expiry date).' }),
            },
        };
    }

    if (typeof dbCode.useLimit === 'number' && (dbCode.useCount || 0) >= dbCode.useLimit) {
        return {
            isValid: false,
            errorResponse: {
                statusCode: 403,
                body: JSON.stringify({ message: 'Access limit reached for this code.' }),
            },
        };
    }
...
export const createSignedUrl = async (event: any, context: ScriptContext): Promise<LambdaResponse> => {
    ...
    const validation = await validateSecretCode(codeFromQuery, context.db, context.now);
    ...
    try {
        ...
        return {
            ...
            body: JSON.stringify({
                // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
                message: error?.message || 'Internal server error while generating signed URL.',
            }),
        };
    }
};